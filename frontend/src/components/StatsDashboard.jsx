  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        {/* Total revenue */}
        <div className="bg-white dark:bg-dark-900 border border-slate-200 dark:border-dark-850 p-5 rounded-2xl shadow-sm flex items-center justify-between transition-transform hover:scale-[1.02]">
          <div>
            <span className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider block">Total Ventas</span>
        ...
        <div className="bg-white dark:bg-dark-900 border border-slate-200 dark:border-dark-850 p-5 rounded-2xl shadow-sm flex items-center justify-between transition-transform hover:scale-[1.02]">
          <div>
            <span className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider block">Pedidos Totales</span>
            <span className="text-2xl font-extrabold text-slate-800 dark:text-slate-100 mt-1 block">
              {kpis?.total_orders || 0}
            </span>
          </div>
        ...
        {/* Pending shipping */}
        <div className="bg-white dark:bg-dark-900 border border-slate-200/60 dark:border-dark-850 p-5 rounded-2xl shadow-sm flex items-center justify-between transition-transform hover:scale-[1.02]">
          <div>
            <span className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider block">Pedidos Pendientes</span>
            <span className="text-2xl font-extrabold text-slate-800 dark:text-slate-100 mt-1 block">
        ...
        <div className="bg-white dark:bg-dark-900 border border-slate-200/60 dark:border-dark-850 p-5 rounded-2xl shadow-sm flex items-center justify-between transition-transform hover:scale-[1.02]">
          <div>
            <span className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider block">Stock Crítico</span>
            <span className="text-2xl font-extrabold text-slate-800 dark:text-slate-100 mt-1 block">
              {kpis?.low_stock || 0} <span className="text-xs font-normal text-slate-400">({kpis?.out_of_stock} agotados)</span>
            </span>
          </div>
          <div className={`p-3 rounded-xl ${kpis?.low_stock > 0 ? 'bg-red-500/15 text-red-500 animate-pulse' : 'bg-slate-100 dark:bg-dark-800 text-slate-400'}`}>
            <ShieldAlert size={24} />
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-dark-900 border border-slate-200 dark:border-dark-850 rounded-2xl p-5 shadow-sm lg:col-span-2">
          <h4 className="text-sm font-bold text-slate-800 dark:text-slate-200 mb-4 uppercase tracking-wider">Tendencia de Ingresos Diarios (S/)</h4>
        ...
        <div className="bg-white dark:bg-dark-900 border border-slate-200 dark:border-dark-850 rounded-2xl p-5 shadow-sm">
          <h4 className="text-sm font-bold text-slate-800 dark:text-slate-200 mb-4 uppercase tracking-wider">Tasa de Conversión por SKU (%)</h4>
          <div className="h-64 relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-dark-900 border border-slate-200 dark:border-dark-850 rounded-2xl p-5 shadow-sm lg:col-span-2 overflow-x-auto">
          <h4 className="text-sm font-bold text-slate-800 dark:text-slate-200 mb-4 uppercase tracking-wider flex items-center gap-1.5">
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-dark-800/40 text-slate-700 dark:text-slate-350">
              {product_analytics?.map((item) => (
                <tr key={item.product_id} className="hover:bg-slate-50 dark:hover:bg-dark-950/20">
                  <td className="py-3 font-semibold">
                    <p className="text-[13px] text-slate-800 dark:text-slate-200">{item.product_name}</p>
                  ...
                  <td className="py-3 text-right">
                    <span className="inline-block px-2 py-0.5 rounded font-extrabold bg-purple-500/10 text-purple-600 dark:text-purple-400">
        ...
        <div className="bg-white dark:bg-dark-900 border border-slate-200 dark:border-dark-850 rounded-2xl p-5 shadow-sm flex flex-col h-[350px]">
          <h4 className="text-sm font-bold text-slate-800 dark:text-slate-200 mb-3 uppercase tracking-wider flex items-center gap-1.5">
          ...
          <div className="flex-1 overflow-y-auto space-y-3.5 pr-2.5 scrollbar-thin">
            {recent_activity?.map((log) => {
              ...
              const styleMap = {
                ...
                "Ver Categoría": "bg-purple-100 text-purple-600 dark:bg-purple-950 dark:text-purple-400"
              };
              
              const relativeTime = new Date(log.timestamp).toLocaleTimeString('es-PE', { hour: '2-digit', minute: '2-digit', second: '2-digit' });

              return (
                <div key={log.id} className="text-[11px] p-2.5 border border-slate-100 dark:border-dark-850 rounded-xl bg-slate-50/50 dark:bg-dark-950/20 hover:bg-white dark:hover:bg-dark-900 transition-all flex flex-col gap-1 shadow-sm">
                  ...
                  <div className="flex items-center justify-between gap-1.5 mt-0.5">
                    ...
                    {log.product_name && (
                      <span className="text-slate-500 dark:text-slate-400 truncate max-w-[120px] font-semibold text-right" title={log.product_name}>
                        {log.product_name}
                      </span>
                    )}
                    {log.category_name && (
                      <span className="text-slate-500 dark:text-slate-400 truncate max-w-[120px]" title={log.category_name}>
